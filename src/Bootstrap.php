...
use F4fShopIntegration\Common\Traits\Requester;
use F4fShopIntegration\Common\Utils\Errors;
use F4fShopIntegration\Config\Classes;
use F4fShopIntegration\Config\I18n;
use F4fShopIntegration\Config\Requirements;
...
final class Bootstrap extends Base {
	...
	public function initClasses() {
		$this->class_list = \apply_filters( 'f4f_shop_integration_initialized_classes', $this->class_list );
		foreach ( $this->class_list as $class ) {
			try {
				...
				\do_action( 'f4f_shop_integration_class_initialize_failed', $err, $class );
				Errors::wpDie(
					...
					__( 'Plugin initialize failed', 'f4f-shop-integration' ),
					__FILE__, $err
				);
			}
		}
	...
	public function getClasses( string $namespace ): array {
		...
		// Errors::writeLog(
		//    [
		//        'title'   => __( 'F4F Shop Integration classes are not being loaded by Composer\'s Autoloader' ),
